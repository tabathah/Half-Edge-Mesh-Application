...
ShaderProgram::ShaderProgram(GLWidget277 *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifTime(-1), unifRenderMode(-1), unifCircleMode(-1),
      unifBindMats(-1), unifTransMats(-1), attrIndex(-1), attrWeight(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    unifCircleMode = context->glGetUniformLocation(prog, "u_CircleMode");

    unifBindMats = context->glGetUniformLocation(prog, "u_BindMats");
    unifTransMats = context->glGetUniformLocation(prog, "u_TransMats");
    attrIndex = context->glGetAttribLocation(prog, "vs_Matrix_Ids");
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrIndex != -1 && d.bindID()) {
        context->glEnableVertexAttribArray(attrIndex);
        context->glVertexAttribIPointer(attrIndex, 2, GL_INT, 0, NULL);
    }

    if (attrWeight != -1 && d.bindWeight()) {
        context->glEnableVertexAttribArray(attrWeight);
        context->glVertexAttribPointer(attrWeight, 2, GL_FLOAT, false, 0, NULL);
    }
    ...
    if (attrIndex != -1) context->glDisableVertexAttribArray(attrIndex);
