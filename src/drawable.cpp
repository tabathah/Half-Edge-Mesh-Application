...
Drawable::Drawable(GLWidget277* context)
    : bufIdx(), bufPos(), bufNor(), bufCol(), bufID(), bufWeight(),
      idxBound(false), posBound(false), norBound(false), colBound(false), idBound(false), weightBound(false),
      context(context)
{}
...
void Drawable::destroy()
{
    ...
    context->glDeleteBuffers(1, &bufID);
    context->glDeleteBuffers(1, &bufWeight);
}
...
{
    idBound = true;
    context->glGenBuffers(1, &bufID);
}

void Drawable::generateWeight()
{
    weightBound = true;
    context->glGenBuffers(1, &bufWeight);
}
